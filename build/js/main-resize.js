...
var multiItemSlider = (function () {
  ...
  return function (selector) {
    ...
    // часть кода для замены путей к изображениям слайдера

    var resizeImage = function () {
      var active = setActive();

      if (active.type === 'mobile') {
        for (var p = 0; p < sliderItems.length; p++) {

          if (sliderItems[p].querySelector('img')) {
            var itemImage = sliderItems[p].querySelector('img');
            var currentSrc = itemImage.getAttribute('src');
            var newSrc = currentSrc.replace(TABLET || DESKTOP, MOBILE);
            itemImage.setAttribute('src', newSrc);
          }
          // return;
        }
      }
